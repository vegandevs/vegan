...
- Several Lattice functions that were deprecated in release 2.7-3 are
  ...
  results of `poolaccum`, `renyi` and `renyiaccum`, `permulattice` with
  `densityplot` and `qqmath` functions for `permustats`. CRAN package
  **ggvegan** provides `autoplot` for all these deprecated functions.
