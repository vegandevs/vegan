...
* **vegan** is now more robust with community data frames with
  'automatic' row names. 'Automatic' names do not actually exist but
  data frames only have information on the number of row names needed,
  and names are generated when they are queried. This is very commonly
  used in `tibble`. However, the row names are not generated by
  default when the data frame is changed to matrix for numerical
  analysis. Many methods in **vegan** assume that there are row names,
  in particular in ordination methods, and many functions failed with
  'automatic' row names (`tibble`) and **vegan** could not be built or
  checked with them. Now the row names are forced in conversion to
  matrix and this should remove possible problems in using `tibble`.
