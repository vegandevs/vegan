...
* **vegan** is now more robust with community data frames with
  'automatic' row names. 'Automatic' row names have been available
  since **R** 2.5.0. 'Automatic' names do not actually exist but data
  frames only have information on the number of row names needed, and
  names are generated automatically when they are queried. This is
  ...
  row names, in particular in ordination methods, and many functions failed
  with 'automatic' row names (`tibble`) and **vegan** could not be
  built or checked with them. Now the row names are forced in
  conversion to matrix and this should remove possible problems in
  using `tibble`.
