...
is performed by function \pkg{vegan} function \code{monoMDS}.
...
the plot in layers using pipe (\code{|>}). Pipe will pass the
ordination object to the next command, and this allows building the
plot in layers:
<<a>>=
plot(ord, type = "n") |>
...
\begin{itemize}
  ...
  \item \pkg{vegan3d} in CRAN provides static 3D plots
    ...
    \item \pkg{ggvegan} in github provides \pkg{ggplot2} graphics for
      ...
      \code{ggplot2} graphics.  (There are some \pkg{lattice} graphics
      functions in \pkg{vegan}, but the plan is to phase out these in
      favour of \code{ggvegan} graphics.)
...
ordination, but it returns the result of the fitted \code{gam}
(Fig. \ref{fig:envfit}):
<<b,fig=false>>=
...
The model gave a message that some constraints were alaised because
...
this case \code{Manure\^4} (fourth degree polynomial of \code{Manure}
level which was defined as an ordered factor). There is one
\code{Manure} level (0) which only occurs in \code{Management} level
\code{NM} (natural management), and we know that \code{Manure} level
once we know the \code{Management}.
