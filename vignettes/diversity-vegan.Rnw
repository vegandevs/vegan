...
the pool size $S_p$. Chao estimator \citep{Chao87, ChiuEtal14} is:
\begin{equation}
\label{eq:chao}
S_p = S_o + \begin{cases} 
...
\begin{align}
S_p &= S_o + f_1 \frac{N-1}{N}  \\ 
S_p & = S_o + f_1 \frac{2N-3}{N}  + f_2 \frac{(N-2)^2}{N(N-1)}
\end{align}
The boostrap estimator \citep{SmithVanBelle84} is:
\begin{equation}
S_p = S_o + \sum_{i=1}^{S_o} (1-p_i)^N
\end{equation}
In all these, $S_o$ is the observed number of species, $f_1$ and $f_2$ are the
...
s^2 = f_1 \left(A^2 \frac{G^3}{4} + A^2 G^2 + A \frac{G}{2} \right),\\
...
\begin{multline}
 ...
 s^2 = \frac{a_1(a_1-1)}{2} + \frac{a_1(2 a_1+1)^2}{(a_2+1)^2}\\
  + \frac{a_1^2 a_2 (a_1 -1)^2}{4 (a_2 + 1)^4} \end{multline}
...
\textsc{ace} is based on rare species also:
\begin{equation}
\begin{split}
