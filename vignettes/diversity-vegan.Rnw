...
S_p &= S_o + \frac{f_1 (f_1 -1)}{f_2 (f_2+1)}  \frac{N-1}{N} & \text{Chao bias-corrected}\\
...
the same data, we miss as many species as we missed
originally. Function \code {specpool} normally applies the basic Chao
estimate, but when there are no doubletons ($f_2 = 0$) it switches to
the bias-corrected form which then reduces to $S_p = S_o + f_1 (f_1
-1) \frac{1}{2} \frac{N}{N-1}$.  \citet{ChiuEtal14} introduced the small-sample
correction term $\frac{N}{N-1}$, but it was not originally used
\citep{Chao87}.
...
\end{multline}
The variance of bias-corrected Chao estimate is \citep{ChiuEtal14}:
\begin{multline}
s^2 = \frac{1}{4} \frac{(N-1)^2}{N^2} f_1 (2f_1 -1)^2 \\+ \frac{1}{2} f_1 (f_1-1) - \frac{1}{4} \frac{f_1^4}{S_p}
\end{multline}
...
estimateR(BCI[k,])
@ 
Chao's method is similar as the bias-corrected model above, but it
uses counts of individuals instead of incidences.  \textsc{ace} is
