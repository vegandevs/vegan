...
models for pool estimate the number of species missing in the sample
$f_0$,.  

Function \code{specpool} implements the following models to estimate
the number of missing species $f_0$. Chao estimator \citep{Chao87, ChiuEtal14} is:
\begin{equation}
\label{eq:chao}
\hat f_0 = \begin{cases} 
...
\end{cases}
\end{equation}
The latter case for $f_2=0$ is known as the bias-corrected form. The
first and second order jackknife estimators \citep{SmithVanBelle84} are:
\begin{align}
\hat f_0 &=  f_1 \frac{N-1}{N}  \\ 
\hat f_0 & =  f_1 \frac{2N-3}{N}  + f_2 \frac{(N-2)^2}{N(N-1)}
\end{align}
The boostrap estimator \citep{SmithVanBelle84} is:
\begin{equation}
...
species as we missed originally.  \citet{ChiuEtal14} introduced the
small-sample correction term $\frac{N}{N-1}$, but it was not
originally used \citep{Chao87}.
...
\end{multline}
These estimators only concern the number of unseen species like previously.

The \textsc{ace} is estimator is defined as:
\begin{equation}
\begin{split}
