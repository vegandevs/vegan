...
`predict` functions are used for **vegan** RDA results. In general,
**vegan** should be able to work normally if **vegan** was loaded
after **klaR**, but if **klaR** was loaded later, its functions will
take precedence over **vegan**. You should check your loaded packages
(**klaR** may be also loaded indirectly via other packages), and take
care that **vegan** is loaded later or remove **klaR** if you do not
need it and you really want to use **vegan** functions.

If you have a very old version of **ade4** (prior to 1.7-8), you may
have similar name clashes with `cca`. The solution is to upgrade
**ade4**.
