...
/** TEMPORARY FUNCTION FOR TESTING & TIMING **/

SEXP test_2x2(SEXP x, SEXP n)
{
    int i, nr = nrows(x), nsim = asInteger(n), len = nr * ncols(x) - 1;
    SEXP out = PROTECT(allocMatrix(INTSXP, 4, nsim));
    int *iout = INTEGER(out);
    for(i = 0; i < nsim; i++)
	get2x2(len, nr, iout + i*4);
    UNPROTECT(1);
    return out;
}

/* The next two functions replicate RNG schemes for timing */

/* Current scheme in swap and quasiswap: get max three random numbers,
 * but often two */

SEXP test_3minus(SEXP x, SEXP n)
{
    int nr = nrows(x), nc = ncols(x), nsim = asInteger(n);
    int i, i4, r0, r1, c0, c1;
    SEXP out = PROTECT(allocMatrix(INTSXP, 4, nsim));
    int *iout = INTEGER(out);
    if (TYPEOF(x) != INTSXP)
	x = coerceVector(x, INTSXP);
    int *ix = INTEGER(x);
    memset(iout, 0, 4 * nsim * sizeof(int));
    for(i = 0; i < nsim; i++) {
	i4 = 4 * i;
	iout[i4] = IRAND(nr * nc - 1);
	r0 = iout[i4] % nr;
	c0 = iout[i4] / nr;
	do {r1 = IRAND(nr - 1); } while (r1 == r0);
	iout[i4+1] = INDX(r1, c0, nr);
	if (ix[iout[i4]] == 0 && ix[iout[i4+1]] == 0)
	    continue;
	do {c1 = IRAND(nc - 1); } while (c1 == c0);
	iout[i4+2] = INDX(r0, c1, nr);
	iout[i4+3] = INDX(r1, c1, nr);
    }
    UNPROTECT(1);
    return out;
}

/* The original method of getting two row and two column indices */

SEXP test_4(SEXP x, SEXP n)
{
    int i, i4, nr = nrows(x), nc = ncols(x), nsim = asInteger(n),
	row[2], col[2];
    SEXP out = PROTECT(allocMatrix(INTSXP, 4, nsim));
    int *iout = INTEGER(out);
    for (i = 0, i4 = 0; i < nsim; i++) {
	I2RAND(row, nr-1);
	I2RAND(col, nc-1);
	iout[i4++] = INDX(row[0], col[0], nr);
	iout[i4++] = INDX(row[1], col[0], nr);
	iout[i4++] = INDX(row[0], col[1], nr);
	iout[i4++] = INDX(row[1], col[1], nr);
    }
    UNPROTECT(1);
    return out;
}
