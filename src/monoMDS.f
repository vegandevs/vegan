...
C
!$OMP PARALLEL DO NUM_THREADS(NTHREADS) PRIVATE(I) SHARED(DIST,NDIS)
      DO I=1,NDIS
        DIST(I)=0.0
      ENDDO
!$OMP END PARALLEL DO
...
!$OMP+ SHARED(X,NDIM,NDIS,DIST) PRIVATE(IDIM,K)
      DO IDIM=1,NDIM
        DO K=1,NDIS
...
C
!$OMP PARALLEL DO NUM_THREADS(NTHREADS) SHARED(DIST,NDIS) PRIVATE(I)
      DO I=1,NDIS
        DIST(I)=SQRT(DIST(I))
      ENDDO
!$OMP END PARALLEL DO
